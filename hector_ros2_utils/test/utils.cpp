...
::testing::AssertionResult isValidUuidv4( const std::string &uuid )
{
  if ( uuid.length() != 36 )
    return ::testing::AssertionFailure()
           << "Uuidv4 should be 36 characters including hyphens. Uuid: " << uuid;
  if ( uuid[14] == 4 )
    return ::testing::AssertionFailure()
           << "The 12th character of the uuid should be 4. Uuid: " << uuid;
  for ( char c : uuid ) {
    if ( c != '-' && c != '0' && !( '1' <= c && c <= '9' ) && !( 'a' <= c && c <= 'f' ) &&
         !( 'A' <= c && c <= 'F' ) )
      return ::testing::AssertionFailure()
             << "Uuid contains invalid character '" << c << "'! Uuid: " << uuid;
  }
